...
import {
  LogOut, Check, X, Edit3, Camera, ChevronLeft, Clock, ChevronRight
...
export const ProfileScreen: React.FC<ProfileClaudeProps> = ({ userId, source = 'Circle', navigation: navProp, isInModal = false }) => {
  ...
  useEffect(() => {
    ...
    if (isOwnProfile) {
      // IMPORTANT: Fetch profile data from database even for own profile to get bio
        ...
        <View style={styles.scrollViewWrapper}>
          ...
          >
            {activeTab === 'profile' ? (
              <>
                {/* Profile Tab Content */}
                {/* Today's Progress Bar - NEW */}
                <View style={styles.todayProgress}>
              <View style={styles.progressHeader}>
                <Text style={styles.progressText}>
                  Today: {profileCompletedActions.length}/{profileActions.length} complete
                </Text>
            ...
            {/* User Posts Feed Section - Outside sectionCard for full width */}
            {postsLoading ? (
              <View style={[styles.postsLoadingContainer, { marginHorizontal: 16 }]}>
                <ActivityIndicator size="small" color="#FFD700" />
              </View>
            ) : userPosts.length > 0 ? (
              <View style={styles.postsContainer}>
                {userPosts.slice(0, 5).map((post, index) => {
                  // Transform the post data to match UnifiedActivityCard's expected format
                ...
                <Pressable
                  style={[styles.loadMoreButton, { marginHorizontal: 16 }]}
                  onPress={() => {
                    if (Platform.OS !== 'web') {
                      Haptics.impactAsync(Haptics.ImpactFeedbackStyle.Light);
                    }
                ...
                >
                  <Text style={styles.loadMoreText}>View All Posts</Text>
                  <ChevronRight size={16} color="#FFD700" />
                </Pressable>
              </View>
            ) : (
              <View style={[styles.noPostsContainer, { marginHorizontal: 16 }]}>
                <Text style={styles.noPostsText}>No posts yet</Text>
                <Text style={styles.noPostsSubtext}>
                  {isOwnProfile ? "Share your progress to inspire others!" : "This user hasn't posted yet"}
                </Text>
              </View>
            )}

            {/* Public Goals Section - REPLACING Active Goals */}
              ...
              <View style={[styles.postsTabContainer, { paddingHorizontal: 8 }]}>
                {postsLoading ? (
                  ...
                  <View style={styles.noPostsContainer}>
                    <Text style={styles.noPostsEmptyTitle}>No Posts Yet</Text>
                    <Text style={styles.noPostsEmptyText}>
                      {isOwnProfile
...
const styles = StyleSheet.create({
  ...
  scrollContent: {
    paddingHorizontal: 0,  // Match SocialV6's no padding for full width cards
  },
  postsContainer: {
    paddingHorizontal: 8,  // Small padding for slight edge separation
  },
  ...
  sectionCard: {
    ...
    overflow: 'hidden',
    marginBottom: 20,
    marginHorizontal: 16,  // Add horizontal margins since we removed scrollContent padding
  },
  ...
  // Public Goals Section additions
  goalsSectionHeader: {
    marginBottom: 12,
  },
  consistencyHint: {
