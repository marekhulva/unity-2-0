...
import { ProfilePostsTimeline } from '../../components/ProfilePostsTimeline';
...
export const ProfileClaude: React.FC<ProfileClaudeProps> = ({ userId, source = 'Circle', navigation: navProp, isInModal = false }) => {
  ...
  // State for tabs
  const [activeTab, setActiveTab] = useState<'posts' | 'activities' | 'goals'>('posts');
  ...
  const createSamplePosts = async () => {
    try {
      const { supabase } = await import('../../services/supabase.service');
      console.log('Creating sample posts for user:', currentUser?.id);

      const samplePosts = [
        {
          user_id: currentUser?.id,
          content: "Just completed my morning workout! 💪 Feeling energized and ready to tackle the day.",
          type: 'activity',
          activity_title: 'Morning Workout',
          visibility: 'circle',
          is_celebration: true,
        },
        {
          user_id: currentUser?.id,
          content: "Day 15 of 75 HARD - Still going strong! The mental toughness is building every single day.",
          type: 'milestone',
          visibility: 'followers',
          metadata: { day: 15, program: '75 HARD' },
        },
        {
          user_id: currentUser?.id,
          content: "Reading 10 pages done ✅ Currently reading 'Atomic Habits' - game changer!",
          type: 'text',
          visibility: 'circle',
        },
        {
          user_id: currentUser?.id,
          content: "Outdoor workout complete despite the rain! No excuses, no matter the weather 🌧️",
          type: 'activity',
          activity_title: 'Outdoor Workout',
          visibility: 'followers',
        },
        {
          user_id: currentUser?.id,
          content: "Nutrition on point today! Prepped all meals for tomorrow. Failing to plan is planning to fail 🥗",
          type: 'text',
          visibility: 'circle',
        },
      ];
      ...
      Alert.alert('Success', 'Sample posts created! Pull to refresh to see them.');
    } catch (error) {
  ...
  return (
    <View style={styles.outerContainer}>
      <View style={styles.innerContainer}>
        ...
        <ProfileHighlights
          highlights={[
            { id: 'streak', title: `${currentStreak}d`, icon: '🔥', color: '#FF6B6B' },
            { id: 'perfect', title: 'Perfect', icon: '⭐', color: '#FFD700', count: perfectDays },
            { id: 'goals', title: 'Goals', icon: '🎯', color: '#4ECDC4', count: goalsCompleted },
            { id: 'weekly', title: `${thisWeekPercentage}%`, icon: '📊', color: '#95E1D3' },
            { id: 'status', title: userStatus, icon: '🏆', color: '#F38181' },
          ]}
          onHighlightPress={(id) => {
            console.log('Highlight pressed:', id);
            // Could open detailed view or stats
            // Temporary: Create sample posts when tapping the status highlight
            if (id === 'status' && isOwnProfile) {
              createSamplePosts();
            }
          }}
        />
