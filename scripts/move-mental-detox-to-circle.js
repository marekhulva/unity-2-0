...
async function moveChallengeToCircle() {
  const circleId = 'bb426edb-a3de-41c3-bdff-3cdcda403088';

  console.log('🔄 Moving Mental Detox challenge to Jing Optimizers circle...\n');

  // Delete the global challenge
  const { error: deleteError } = await supabase
    .from('challenges')
    .delete()
    .eq('name', '7 Day Mental Detox')
    .eq('scope', 'global');

  if (deleteError) {
    console.error('❌ Error deleting global challenge:', deleteError.message);
  } else {
    console.log('✅ Deleted global challenge');
  }
...
Remember: You need 32/42 checkmarks to succeed (75%). That's 6 per day on average. Missing a few is OK - this is about progress, not perfection.`,
  ...
  if (error) {
    console.error('❌ Error creating circle challenge:', error.message);
    process.exit(1);
  }

  console.log('✅ Challenge created in Jing Optimizers circle!');
  console.log('\n📋 Details:');
  console.log('   ID:', data.id);
  console.log('   Name:', data.name);
  console.log('   Scope:', data.scope);
  console.log('   Circle ID:', data.circle_id);
  console.log('\n✨ Done! Check port 8081 to see it in Jing Optimizers.');
}

moveChallengeToCircle();
